     ...
     function(input, output){
         ...
         output$gender <- renderText({
          ...
          output$dt <- renderDataTable(
               data, options = list(iDisplayLength = 10))
          
          # 3rd Tab
          output$Plot1 <- renderChart({
               p1 <- hPlot(Estimate~Age, 
                           data = subset(data, Gender=="Males"), 
                           type = c("line", "bubble", "scatter"), 
                           group = "College_Enrollment", 
                           size="Estimate")
          ...
          output$Plot2 <- renderChart({
               p1 <- hPlot(Estimate~Age, 
                           data = subset(data, Gender=="Females"), 
                           type = c("line", "bubble", "scatter"), 
                           group = "College_Enrollment", size="Estimate")
